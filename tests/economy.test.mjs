...
import {newGame,reducer,unlocked,highestSeed,PLANTS,SEED_UNLOCK,seedPlantId,teamFor} from '../src/game.ts'
import {employ} from './helpers.mjs'
test('all eight seeds respect wallet balance, with sufficient permanent yield',()=>{
 for(let tier=0;tier<8;tier++){
  const s=newGame(),id=seedPlantId(tier),cost=PLANTS[id].cost;s.coins=cost;s.earned=SEED_UNLOCK[tier];s.upgrades.profit=12
  assert.equal(s.purchases.length,0);assert.equal(unlocked(s,tier),true)
...
test('wallet helper finds affordable tiers while squirrels use the garden selection',()=>{
 for(const [coins,tier] of [[0,0],[49,0],[50,1],[499,1],[500,2],[10000,3],[500000,4],[50000000,5],[10000000000,6]]){const s=newGame();s.upgrades.profit=12;s.coins=coins;s.earned=coins;assert.equal(highestSeed(s),tier)}
 let s=reducer(newGame(),{type:'start'});s.coins=1000;s.earned=500;s=employ(s,'sow');s.coins=500;s=reducer(s,{type:'sow-tier',tier:2})
