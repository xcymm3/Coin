...
## 原创背景音乐与像素特效（2026-09-15）

- 《月光下，慢慢生长》：72 BPM，C / Am / F / G 和声，64 拍（约 53 秒）旋律循环。柔和正弦拨音、低音和弦与稀疏钟琴泛音由 Web Audio 实时合成，无外部音乐素材或网络音频下载。
- 音乐与音效独立总线、音量和开关；渐变静音、动态压缩、55 ms 音效触发限流。后台暂停 AudioContext 与调度器，返回后重建调度时间，音符结束即断开节点。
- 美术继续复用原像素图集；新增硬边方形水滴、泥土、金币粒子、植物轻摆与成熟弹出、三档光环、星芒、森林萤火与蜗牛爬行。尊重系统减少动态效果设置，并支持游戏内关闭。


## 六品阶新增植物

`src/plantArt.ts` 保存新增 11 种植物的独立像素轮廓、配色和幼芽；`src/ExtraPlant.tsx` 用 crispEdges SVG 绘制成株，并复用原有图集的空花盆材质。原十种植物继续使用原图集，未替换植物与道具的图集编号。新增图形由项目代码直接绘制，无外部素材或运行时资源依赖。
