...
import {newGame,reducer,PLANTS,UPGRADES,seedPlantId,teamFor,hireCatalog,hireAvailable,expansionLock,GARDEN_PRICES,potPrice,seedEconomy,reward,SEED_PRICES} from '../src/game.ts';
export function runCampaign(seed=42,interval=2,{noExpand=false,limit=9000,fertilizer=true}={}){
 ...
 function bestSeed(page){let best=0,score=0;for(let t=0;t<7;t++){if(s.coins<SEED_PRICES[t]*2)continue;const e=seedEconomy(t);const net=e.gross*(reward(s,PLANTS[0],page)/PLANTS[0].reward)-SEED_PRICES[t];const rate=net/e.seconds;if(rate>score){score=rate;best=t;}}return best;}
 for(let t=0;t<limit&&s.wonAt===null;t++){
  if(t%interval===0){actions++;if(t<300)earlyActions++;let acted=false;const page=s.activeGarden,team=teamFor(s),indices=Array.from({length:team.potCount},(_,i)=>page*15+i),star=indices.find(i=>s.pots[i].plant===9),tier=bestSeed(page);
   ...
   if(star!==undefined){if(s.player.phase==='idle' && s.elapsed-s.pots[star].wateredAt>=5 && !s.pots[star].watering){s=reducer(s,s.player.stock===0?{type:'refill'}:{type:'water',index:star});acted=true;}}
