...
export type Upgrade = {id:string;page:number;name:string;cost:number;icon:string;detail:string;effects:Partial<Record<EffectId,number>>;requires?:string;harvests?:number}
...
export const GARDENS = [
 ...
 { name:'萤火溪谷', subtitle:'溪水绕过发光的蘑菇', reward:5, growth:1.2, theme:'brook', ornament:'stream' },
 { name:'琥珀沙庭', subtitle:'暖风与金色的石阶', reward:25, growth:1.5, theme:'amber', ornament:'sundial' },
 { name:'极光雪苑', subtitle:'冰晶映着缓缓流动的极光', reward:125, growth:2, theme:'aurora', ornament:'crystal' },
 { name:'星海天台', subtitle:'在星轨下种出最后一朵花', reward:625, growth:2.5, theme:'astral', ornament:'orrery' },
]

// Harvest research is local; soil and manual watering research remain shared.
export const UPGRADE_BASES = [270,660000,20000000,500000000,15000000000]
...
export const UPGRADES: Upgrade[] = GARDENS.flatMap((_,page)=>[
 ...[.15,.25,.35,.5].map((ratio,level)=>({id:level===3?`g${page}-profit`:`g${page}-profit-${level+1}`,page,name:`${names[page][0]} ${level+1}/4`,cost:page===0?[40,65,85,110][level]:Math.round(UPGRADE_BASES[page]*ratio),icon:'coin',detail:'本园收获价值提升约 41% · 分四级研究',effects:{profit:.5},requires:level?`g${page}-profit-${level}`:undefined,harvests:[0,12,30,60][level]})),
...
export const GARDEN_PRICES=[0,330000,9000000,360000000,12500000000]
export const HIRE_BASES=[60,1800,40000,800000,25000000]
