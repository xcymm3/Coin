...
import { PLANTS, TIERS, UPGRADES, SAVE_KEY, newGame, parseSave, reducer, unlocked, price, reward, upgradePrice, clickPower, growthRate, formatTime, capacity, STATIONS, WATER_DURATION, SEED_UNLOCK, upgradeLock, isGerminating, GERMINATION_SECONDS, plantChance, ULTIMATE_ID, ULTIMATE_TIER, TIER_PLANTS, seedPlantId, seedEconomy, GARDENS, gardenCount, teamFor, workerDuration, hireCatalog, hireAvailable, crewFor, MATERIALS, CREW_NAMES, decorationPrice, GARDEN_PRICES, EXPANSION_PRICE, expansionLock, ULTIMATE_PURCHASES, VARIANTS, DECORATIONS, WEATHER, type Tier, type GameState } from './game'
...
export default function Garden({ initialState, persist = true }: { initialState?: GameState; persist?: boolean } = {}) {
  ...
  const nextGoal = s.wonAt !== null ? '星之花已经绽放。继续创造你的奇妙花园吧。' : nextTier >= 0 ? `累计收益 ${number(SEED_UNLOCK[nextTier])} 金币，解锁${TIERS[nextTier]}${nextTier === ULTIMATE_TIER ? ` · 研究 ${s.purchases.length}/${ULTIMATE_PURCHASES} · 五页花园` : ''}` : star ? '照料永恒星之花，让第一颗星星在花园绽放' : `积攒 ${number(PLANTS[ULTIMATE_ID].cost)} 金币，种下永恒星之花`
  ...
  return <div className={`game-shell ${!settings.motion ? 'reduce-motion' : ''} ${screen === 'menu' ? 'on-menu' : ''} ${paused ? 'is-paused' : ''} ${observing ? 'observation-mode' : ''}`}>
    ...
    <main className="game-layout">
      <aside className={`seed-panel wood ${mobilePanel === 'seeds' ? 'mobile-active' : ''}`} aria-label="种子商店">
        ...
        <div className="tier-list">{TIERS.map((name, i) => {
          ...
          return <button key={name} className={`tier-card blue-button tier-${i} ${active ? 'selected' : ''}`} aria-pressed={active} disabled={!open} onClick={() => choose(seedPlantId(i as Tier))}>
            ...
            <span className="tier-copy"><strong>{name}</strong><b>{i === 0 ? '免费 · 无限' : `${number(price(s, PLANTS[seedPlantId(i as Tier)]))} 金币`}</b><small>{open ? i === ULTIMATE_TIER ? '种出星星 · 完成旅程' : `${TIER_PLANTS[i].filter(id => s.harvestCounts[id] > 0).length} / 4 已收获` : i === ULTIMATE_TIER ? `完成 ${ULTIMATE_PURCHASES} 项研究 · 五页花园` : `累计收益 ${number(SEED_UNLOCK[i])} 解锁`}</small></span>
          </button>
        })}</div>
      ...
      <section className={`garden-section ${mobilePanel === 'garden' ? 'mobile-active' : ''}`} aria-label="花园">
        ...
        <div className="garden-expansion">{s.pots.length===10?<button className="blue-button" disabled={s.coins<EXPANSION_PRICE} onClick={()=>{dispatch({type:'expand'});sound('buy')}}>扩建苔庭至 15 盆 · ◈ {number(EXPANSION_PRICE)}</button>:gardenCount(s)<5?<button className="blue-button" disabled={!!expansionLock(s)||s.coins<GARDEN_PRICES[gardenCount(s)]} onClick={()=>{dispatch({type:'open-garden'});setInspectedPot(null);setMoveFrom(null);sound('buy')}}>{expansionLock(s)??`开辟${GARDENS[gardenCount(s)].name} · ◈ ${number(GARDEN_PRICES[gardenCount(s)])}`}</button>:<small>五座花园已开放 · 每座独立组建团队</small>}</div>
        ...
        <div className={`garden-board garden-${garden.theme} wood zen-garden`}>
          ...
          <div className="garden-floor" inert={observing}><svg className="garden-landmark" viewBox="0 0 120 40" preserveAspectRatio={s.activeGarden === 1 ? "none" : "xMidYMid meet"} aria-hidden="true" shapeRendering="crispEdges">{s.activeGarden===0?<path fill="#476848" d="M12 36V8h4v28h-4M4 12h8v4H4m12 4h10v4H16m-12 2h8v4H4"/>:s.activeGarden===1?<><path fill="#428eaa" d="M0 22h26v-8h30v8h30v-9h34v17H86v7H56v-9H26v8H0Z"/><path fill="#a0dfd0" d="M0 23h26v-6h28v4H28v6H0m64 0h22v-6h24v4H90v6H64Z"/></>:s.activeGarden===2?<><path fill="#80552e" d="M41 29h38v8H41zM48 23h24v6H48z"/><path fill="#ead18b" d="M47 18h27v7H47z"/><path fill="#5e4530" d="M59 1h4v21h-4z"/></>:s.activeGarden===3?<><path fill="#58a4b1" d="M30 35V16l8-12 8 12v19zm24 0V12L65 0l11 12v23zm30 0V21l8-10 8 10v14z"/><path fill="#d2fcf0" d="M38 4v26h-4V16zm27-4v30h-5V12zm27 11v21h-4V21z"/></>:<><path fill="none" stroke="#c7a559" strokeWidth="3" d="M38 5h45v23H38zM50 0h20v36H50z"/><path fill="#f5db78" d="M56 12h10v10H56z"/><path fill="#66517c" d="M45 34h34v5H45z"/></>}</svg>
            ...
            if (!pot) return <button key={i} className="locked-pot ground-marker" style={placement} aria-label={`扩建第${i + 1}个花盆`} onClick={() => { setCategory(0); setMobilePanel('upgrades'); setToast(`点击花园上方的扩建按钮，支付 ${EXPANSION_PRICE} 金币扩至十五盆。`) }}><span>＋</span><small>扩建</small></button>
      ...
      <aside ref={shopRef} className={`upgrade-panel ${mobilePanel === 'upgrades' ? 'mobile-active' : ''}`} aria-label="花园商店">
        ...
        <div className="upgrade-list">
          {category===0 && <>{UPGRADES.filter(u=>u.page===s.activeGarden&&!s.purchases.includes(u.id)).map(u=><button key={u.id} data-testid={`upgrade-${u.id}`} className="upgrade-card" disabled={s.coins<upgradePrice(s,u)||!!upgradeLock(s,u.id)} onClick={()=>{dispatch({type:'buy',id:u.id});sound('buy');setToast(`${u.name} · ${u.detail}`)}}><span className="upgrade-art"><Icon name={u.icon}/></span><span className="upgrade-copy"><strong>{u.name}</strong><b>◈ {number(u.cost)}</b><span className="upgrade-description">{u.detail}</span><small>来自{garden.name} · 全园共享</small></span></button>)}{UPGRADES.filter(u=>u.page===s.activeGarden).every(u=>s.purchases.includes(u.id))&&<p className="shop-empty">本园三项专属升级已完成。<br/>可继续招募助手，或开辟下一座花园。</p>}</>}
    ...
    {screen === 'menu' && !panel && <Modal title="月光奇植园开始菜单" className="start-menu">
      ...
      <small className="menu-footnote">21 种奇植 · 五园专属研究 · 独立动物团队 · 慢慢种下自己的星空</small>
    </Modal>}
    ...
    {panel === 'help' && <Modal title="玩法指南" close={() => setPanel(null)}><h2>园丁的小手册</h2><ol className="help-list"><li><b>选种、播种</b><p>六档种子依次为普通、稀有、珍贵、超凡、神话、终极。前五档各有四种植物，普通种子免费；各档均能抽到全部二十种非终极植物，具体概率见商店。各品阶内四种植物概率依次为 20%、50%、25%、5%。终极种子固定种出星之花。前 5 秒有效成长只显示神秘种子，浇水与土壤可加速揭晓；挖除不退种子费用。</p></li><li><b>浇水、收获</b><p>植物会自然生长。不同品种会长出不同幼芽。顶部选择水壶，再点击单株植物播放 1.2 秒浇水动画，期间该株无法重复浇水，但可以同时浇其他植物，每次立即消耗一格充能；点击水壶右侧「装满」补水。默认点击空盆播种、点击成熟植物收获，无需选择专门工具；水壶选中时也可直接播种和收获。铲子选中后可挖除任何阶段的植物，且不获得金币或收获次数。再次点击已选工具可取消选择。选择小推车后先点植物，再点空盆移动或另一株植物交换位置，成长进度会保留。手动或甲虫收获时均有 1% 概率获得一份肥料。选择肥料工具后点击未成熟的非终极植物，消耗一份立即养成；终极、空盆或已成熟植物不会消耗肥料。植物没有额外特殊效果；进度满后再次点击收获金币。</p></li><li><b>扩建、雇用助手</b><p>商店分为升级、雇佣、装饰，只列出尚未购买的选项。每园恰好三项专属升级，购买后对全园生效。每座花园独立雇佣蜗牛、甲虫和松鼠，每种最多五只；木制装备可依次强化为铜制、铁制、金质、钻石制，仅影响本园同种动物。新园不会自动获得动物或装饰。松鼠自动选择最高已解锁的非终极种子，缺钱时用免费种子；甲虫运回金币，蜗牛回池补水。完成最新花园的三项升级后，可付费开辟下一园。</p></li><li><b>种出第一颗星星</b><p>累计收益解锁新种子。完成全部 15 项专属升级并开辟五座花园后，可解锁终极种子。星之花基础成长需 8 分钟，与其他植物一样接受手动浇水、蜗牛浇水、土壤加成。成熟即通关，之后可以继续种植。</p></li></ol><p className="settings-note">装饰仅改变本园外观，高级花园价格更高；购买后可在设置中隐藏。每隔 5–10 分钟出现一次细雨、萤火虫之夜或彩虹，持续 15 秒，不改变收益和成长。星之花不会被自动收获或自动播种。可关闭自动收获，保留喜欢的植物观赏。所有浇水只作用于单株植物。水壶空了请在顶部工具栏装填。观赏模式保留音乐和自动照料，隐藏数值提示与通关弹窗。</p><button className="primary-button" onClick={() => setPanel(null)}>知道了</button></Modal>}
