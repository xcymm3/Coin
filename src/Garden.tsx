        ...
        <div className="garden-tools" role="group" aria-label="园艺工具">
          <div className="tool-slots">
            <div className="watering-cubby">
              <button className={`tool-button water-tool ${tool === 'water' ? 'selected' : ''} ${s.player.phase==='service'?'is-refilling':''}`} data-water-state={waterState} title={`水壶 · ${waterState}。空壶时点击补水；未空时用右下角补水按钮。`} aria-label={`水壶工具 · ${waterState}`} aria-pressed={tool === 'water'} onClick={() => { if(s.player.stock===0){dispatch({type:'refill'});setTool('water')}else setTool(tool==='water'?null:'water');setMoveFrom(null) }}><ToolArt kind="water" waterLevel={waterLevel}/>{s.player.phase==='service'&&<span className="refill-drops" aria-hidden="true">•••</span>}<span>水壶</span></button>
            ...
            <button className={`tool-button ${tool === 'cart' ? 'selected' : ''}`} title="小推车 · 选择植物，切换花园后点击花盆搬运或交换" aria-label="小推车工具" aria-pressed={tool === 'cart'} onClick={() => { setTool(tool === 'cart' ? null : 'cart'); setMoveFrom(null) }}><ToolArt kind="cart"/>{moveFrom !== null && s.pots[moveFrom]?.plant != null && <span className="cart-passenger" aria-hidden="true">{isGerminating(s.pots[moveFrom]) ? <Icon name="seed"/> : <PlantSprite id={s.pots[moveFrom].plant!}/>}</span>}<span>小推车</span></button>
          ...
          <p className="tool-hint">{tool === 'fertilizer' ? '消耗 1 份，让未成熟的非终极植物立即成熟 · 收获时 1% 掉落' : tool === 'shovel' ? '点击挖除任何植物 · 无收益 · 再点铲子取消' : tool === 'cart' ? moveFrom === null ? '选择植物 → 用花园左右箭头跨园 → 点击花盆搬运或交换' : `已选 ${GARDENS[Math.floor(moveFrom/15)].name} · ${moveFrom%15+1} 号盆，可跨园搬运；再点小推车取消` : tool === 'water' ? '观察壶身水色 · 空壶点击补水 · 可轮流浇不同植物' : '点击空盆播种，点击成熟植物收获'}</p>
        </div>
          ...
          {s.elapsed-s.weather.started>=0 && s.elapsed-s.weather.started<15 && <div className={`weather-overlay weather-${s.weather.kind}`} aria-label={`${WEATHER[s.weather.kind]}天气，持续15秒，无数值效果`}><span>{WEATHER[s.weather.kind]} · 15 秒</span>{s.weather.kind===2&&<svg className="pixel-rainbow" viewBox="0 0 64 32" shapeRendering="crispEdges" aria-hidden="true">{["#bba1d3","#8fcbdc","#b0d398","#e6cf88","#dc94a0"].map((color,i)=><path key={color} d={`M${4+i*2} 30V${16+i*2}h6v-6h8V${4+i*2}h${28-i*4}v6h8v6h6V30`} fill="none" stroke={color} strokeWidth="2"/>)}</svg>}{Array.from({length:28},(_,i)=><i key={i} style={{left:`${i*37%100}%`,top:`${i*23%100}%`,animationDelay:`-${i*.19}s`}}/>)}</div>}
