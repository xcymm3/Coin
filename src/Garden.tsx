...
import { PLANTS, TIERS, UPGRADES, SAVE_KEY, newGame, parseSave, reducer, unlocked, price, reward, upgradePrice, clickPower, growthRate, formatTime, capacity, STATIONS, WATER_DURATION, SEED_UNLOCK, upgradeLock, isGerminating, GERMINATION_SECONDS, plantChance, ULTIMATE_ID, ULTIMATE_TIER, TIER_PLANTS, seedPlantId, seedEconomy, GARDENS, gardenCount, teamFor, workerDuration, upgradeChapter, ULTIMATE_PURCHASES, type Tier, type GameState } from './game'
...
export default function Garden({ initialState, persist = true }: { initialState?: GameState; persist?: boolean } = {}) {
  ...
  const [tool, setTool] = useState<'water' | 'cart' | 'shovel' | null>(null)
    ...
    <main className="game-layout">
      ...
      <section className={`garden-section ${mobilePanel === 'garden' ? 'mobile-active' : ''}`} aria-label="花园">
        ...
        <div className="garden-tools" role="group" aria-label="园艺工具">
          ...
          <button className="tool-button" disabled title="肥料功能即将开放"><Icon name="leaf" /><span>肥料<small>未开放</small></span></button>
          ...
          <p className="tool-hint">{tool === 'shovel' ? '点击挖除任何植物 · 无收益 · 再点铲子取消' : tool === 'cart' ? moveFrom === null ? '选择植物 → 点击空盆移动，或点击另一株交换位置' : `已选择花盆 ${moveFrom + 1} · 点击目的地，再点原盆取消` : tool === 'water' ? '每株冷却 1.2 秒 · 可连续浇不同植物 · 每次 1 格充能' : '点击空盆播种，点击成熟植物收获'}</p>
        </div>
      ...
      <aside className={`upgrade-panel ${mobilePanel === 'upgrades' ? 'mobile-active' : ''}`} aria-label="升级商店">
        ...
        <div className="upgrade-tabs" role="tablist" aria-label="升级分类">{['待购买', '下一组', '已获得'].map((name, i) => <button role="tab" aria-selected={category === i} key={name} onClick={() => { setCategory(i); sound('tap') }} className={category === i ? 'selected' : ''}><Icon name={['hand', 'pot', 'snail'][i]} /><span>{name}</span></button>)}</div>
        <div className="upgrade-list">{UPGRADES.filter(u => category === 2 ? s.purchases.includes(u.id) : u.chapter === chapter + category).map(u => {
