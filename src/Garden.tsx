...
import { PLANTS, TIERS, UPGRADES, SAVE_KEY, newGame, parseSave, reducer, unlocked, price, upgradePrice, clickPower, growthRate, formatTime, capacity, STATIONS, WATER_DURATION, upgradeLock, ULTIMATE_ID, ULTIMATE_TIER, TIER_PLANTS, seedPlantId, GARDENS, gardenCount, teamFor, hireCatalog, hireAvailable, hireLock, seedLock, crewFor, MATERIALS, CREW_NAMES, decorationPrice, GARDEN_PRICES, potPrice, expansionLock, variantFor, plantedReward, DECORATIONS, WEATHER, WEATHER_EFFECTS, WEATHER_DURATION, activeWeather, infiniteWater, type Tier, type GameState } from './game'
  ...
  return <div className={`game-shell ${landscapeShopOpen ? 'landscape-shop-open' : 'landscape-shop-closed'} ${!settings.motion ? 'reduce-motion' : ''} ${screen === 'menu' ? 'on-menu' : ''} ${paused ? 'is-paused' : ''} ${observing ? 'observation-mode' : ''}`}>
    ...
    <main className="game-layout">
      ...
      <aside id="garden-shop" ref={shopRef} className={`upgrade-panel ${mobilePanel === 'upgrades' ? 'mobile-active' : ''}`} aria-label="花园商店">
        ...
        <div className="garden-bonuses"><div className="eyebrow">本园团队 · 独立经营</div>{(['water','harvest','sow'] as const).map(kind=><p key={kind}><span>{CREW_NAMES[kind]}</span><b>{crewFor(team,kind).length}/5 · {MATERIALS[team.equipment[kind]].name}</b></p>)}<div className="eyebrow">全园能力 · {s.purchases.length}/{UPGRADES.length} 级研究</div><p><span>手动浇水</span><b>+{clickPower(s)} 秒</b></p><p><span>本页自然成长</span><b>×{growthRate(s).toFixed(1)}</b></p><p><span>本页收获收益</span><b>×{Number((2**s.upgrades.profit*garden.reward).toFixed(2))}</b></p></div>
