...
import { PLANTS, TIERS, UPGRADES, SAVE_KEY, newGame, parseSave, reducer, unlocked, price, upgradePrice, clickPower, growthRate, formatTime, capacity, STATIONS, WATER_DURATION, upgradeLock, ULTIMATE_ID, ULTIMATE_TIER, TIER_PLANTS, seedPlantId, GARDENS, gardenCount, teamFor, hireCatalog, hireAvailable, hireLock, seedLock, SEED_GARDENS, harvestMultiplier, crewFor, MATERIALS, CREW_NAMES, decorationPrice, GARDEN_PRICES, potPrice, expansionLock, variantFor, plantedReward, DECORATIONS, WEATHER, WEATHER_EFFECTS, WEATHER_DURATION, activeWeather, infiniteWater, type Tier, type GameState } from './game'
  ...
  return <div className={`game-shell ${landscapeShopOpen ? 'landscape-shop-open' : 'landscape-shop-closed'} ${!settings.motion ? 'reduce-motion' : ''} ${screen === 'menu' ? 'on-menu' : ''} ${paused ? 'is-paused' : ''} ${observing ? 'observation-mode' : ''}`}>
    ...
    <main className="game-layout">
      ...
      <aside id="garden-shop" ref={shopRef} className={`upgrade-panel ${mobilePanel === 'upgrades' ? 'mobile-active' : ''}`} aria-label="花园商店">
        ...
        <div className="shop-location">{garden.name}<small>{category===0?'丰收仅本园 · 沃土与灌注全园共享':'购买与效果仅限当前花园'}</small></div>
        ...
        <div className="upgrade-list">
          {category===0 && <>{UPGRADES.filter(u=>u.page===s.activeGarden&&!s.purchases.includes(u.id)&&(!u.requires||s.purchases.includes(u.requires))).map(u=><button key={u.id} data-testid={`upgrade-${u.id}`} className="upgrade-card" disabled={s.coins<upgradePrice(s,u)||!!upgradeLock(s,u.id)} onClick={()=>{dispatch({type:'buy',id:u.id});sound('buy');setToast(`${u.name} · ${u.detail}`)}}><span className="upgrade-art"><Icon name={u.icon}/></span><span className="upgrade-copy"><strong>{u.name}</strong><b>◈ {number(u.cost)}</b><span className="upgrade-description">{upgradeLock(s,u.id)??u.detail}</span><small>来自{garden.name} · {u.effects.profit?'仅本园':'全园共享'}</small></span></button>)}{UPGRADES.filter(u=>u.page===s.activeGarden).every(u=>s.purchases.includes(u.id))&&<p className="shop-empty">本园六级专属研究已完成。<br/>可继续招募助手，或开辟下一座花园。</p>}</>}
        ...
        <div className="garden-bonuses"><div className="eyebrow">本园团队 · 独立经营</div>{(['water','harvest','sow'] as const).map(kind=><p key={kind}><span>{CREW_NAMES[kind]}</span><b>{crewFor(team,kind).length}/5 · {MATERIALS[team.equipment[kind]].name}</b></p>)}<div className="eyebrow">研究进度 · {s.purchases.length}/{UPGRADES.length} 级研究</div><p><span>手动浇水</span><b>+{clickPower(s)} 秒</b></p><p><span>本页自然成长</span><b>×{growthRate(s).toFixed(1)}</b></p><p><span>本页收获收益</span><b>×{Number((harvestMultiplier(s)).toFixed(2))}</b></p></div>
    ...
    {panel === 'squirrel' && <Modal title="松鼠播种选种" className="squirrel-seed-modal" close={()=>setPanel(null)}>
      ...
      <div className="squirrel-seed-options">{TIERS.slice(0,ULTIMATE_TIER).map((name,tier)=>{
        ...
        return <button key={tier} className={`blue-button squirrel-seed-option tier-${tier}`} aria-pressed={team.sowTier===tier} disabled={s.activeGarden<SEED_GARDENS[tier]} onClick={()=>{dispatch({type:'sow-tier',tier});sound('tap');setPanel(null)}}>
          <span className="seed-bag"><Sprite id={13}/></span><span><strong>{name}</strong><small>{cost===0?'免费':`${number(cost)} 金币 / 颗`}</small>{lock?<small>{lock}{s.activeGarden>=SEED_GARDENS[tier]?' · 选后等待':''}</small>:s.coins<cost&&<small>金币不足 · 选后等待</small>}</span><b>{team.sowTier===tier?'✓':''}</b>
        </button>
      })}</div>
    </Modal>}
