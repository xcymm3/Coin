...
import { PLANTS, TIERS, UPGRADES, SAVE_KEY, newGame, parseSave, reducer, unlocked, price, reward, upgradePrice, clickPower, growthRate, formatTime, capacity, STATIONS, WATER_DURATION, SEED_UNLOCK, upgradeLock, isGerminating, GERMINATION_SECONDS, plantChance, ULTIMATE_ID, ULTIMATE_TIER, TIER_PLANTS, seedPlantId, seedEconomy, GARDENS, gardenCount, teamFor, workerDuration, upgradeChapter, ULTIMATE_PURCHASES, VARIANTS, DECORATIONS, WEATHER, type Tier, type GameState } from './game'
...
export default function Garden({ initialState, persist = true }: { initialState?: GameState; persist?: boolean } = {}) {
  ...
  const garden = GARDENS[s.activeGarden], team = teamFor(s, s.activeGarden), chapter = upgradeChapter(s)
  ...
  const possibilities = TIER_PLANTS.flat().map(id => PLANTS[id]).filter(p => plantChance(selected.tier, p.id) > 0)
  ...
  function potClick(index: number) {
    ...
    if (tool === 'water' && p.plant !== null && p.growth < PLANTS[p.plant].seconds) {
      ...
      if (s.player.phase !== 'idle') { setToast('水壶正在装填，请稍等。'); return }
      if (s.player.stock === 0) { setToast('水壶没有充能了，点击顶部「装满」补水。'); return }
    ...
    } else if (p.growth >= PLANTS[p.plant].seconds) { text = `+${number(reward(s, PLANTS[p.plant]))}`; sound('coin') }
  ...
  const nextTier = TIERS.findIndex((_, i) => !unlocked(s, i as Tier))
  const star = s.pots.find(p => p.plant === ULTIMATE_ID)
  const nextGoal = s.wonAt !== null ? '星之花已经绽放。继续创造你的奇妙花园吧。' : nextTier >= 0 ? `累计收益 ${number(SEED_UNLOCK[nextTier])} 金币，解锁${TIERS[nextTier]}${nextTier === ULTIMATE_TIER ? ` · 改造 ${s.purchases.length}/${ULTIMATE_PURCHASES} · 五页花园` : ''}` : star ? '照料永恒星之花，让第一颗星星在花园绽放' : `积攒 ${number(PLANTS[ULTIMATE_ID].cost)} 金币，种下永恒星之花`
  const goalProgress = s.wonAt !== null ? 1 : nextTier >= 0 ? (nextTier === ULTIMATE_TIER ? s.purchases.length / ULTIMATE_PURCHASES : (s.earned - SEED_UNLOCK[nextTier - 1]) / (SEED_UNLOCK[nextTier] - SEED_UNLOCK[nextTier - 1])) : star ? star.growth / PLANTS[ULTIMATE_ID].seconds : s.coins / PLANTS[ULTIMATE_ID].cost
  ...
  return <div className={`game-shell ${!settings.motion ? 'reduce-motion' : ''} ${screen === 'menu' ? 'on-menu' : ''} ${paused ? 'is-paused' : ''} ${observing ? 'observation-mode' : ''}`}>
    ...
    <main className="game-layout">
      <aside className={`seed-panel wood ${mobilePanel === 'seeds' ? 'mobile-active' : ''}`} aria-label="种子商店">
        ...
        <div className="tier-list">{TIERS.map((name, i) => {
          ...
          return <button key={name} className={`tier-card blue-button tier-${i} ${active ? 'selected' : ''}`} aria-pressed={active} disabled={!open} onClick={() => choose(seedPlantId(i as Tier))}>
            ...
            <span className="tier-copy"><strong>{name}</strong><b>{i === 0 ? '免费 · 无限' : `${number(price(s, PLANTS[seedPlantId(i as Tier)]))} 金币`}</b><small>{open ? i === ULTIMATE_TIER ? '种出星星 · 完成旅程' : `${TIER_PLANTS[i].filter(id => s.harvestCounts[id] > 0).length} / 4 已收获` : i === ULTIMATE_TIER ? `完成 ${ULTIMATE_PURCHASES} 项改造 · 五页花园` : `累计收益 ${number(SEED_UNLOCK[i])} 解锁`}</small></span>
          </button>
        })}</div>
        <div className="seed-selection"><div className="eyebrow">{selected.tier === ULTIMATE_TIER ? '唯一奇植' : '可能长出的植物'} <span>{selected.tier === ULTIMATE_TIER ? '100%' : '每株概率如下'}</span></div>
          <div className="species-list seed-outcomes">{possibilities.map(p => <div key={p.id} className="species-button"><PlantSprite id={p.id} /><span>{p.name}<small> {(plantChance(selected.tier, p.id) * 100).toFixed(5).replace(/\.?0+$/, "")}%</small></span></div>)}</div>
          <div className="plant-details"><h3>{TIERS[selected.tier]}</h3><div className="plant-numbers"><span>◷ {formatTime(possibilities[0].seconds)}{possibilities.length > 1 ? `–${formatTime(possibilities.at(-1)!.seconds)}` : ''}</span><span className="gold-text">收获 {number(reward(s, possibilities[0]))}{possibilities.length > 1 ? `–${number(reward(s, possibilities.at(-1)!))}` : ''} 金币</span></div><div className="plant-purchase">每包 {price(s, selected) === 0 ? '免费' : `${price(s, selected)} 金币`}</div><p>本页期望净收益 {number(seedEconomy(selected.tier).gross * 2 ** s.upgrades.profit * garden.reward - price(s, selected))} 金币／包</p><p>{selected.tier === ULTIMATE_TIER ? '成熟即通关，享受正常浇水、土壤和初始成长加成。' : '可种出全部二十种非终极植物，概率见上方。前 5 秒有效成长隐藏品种；珍贵及以上植物揭晓时闪光。'}</p></div>
        </div>
      ...
      <section className={`garden-section ${mobilePanel === 'garden' ? 'mobile-active' : ''}`} aria-label="花园">
        <div className="garden-heading"><div><span className="live-dot" /><h2>{garden.name}</h2><span className="garden-count">{Math.min(15,s.pots.length-s.activeGarden*15)} / 15 · 全园 {s.pots.length}/75</span></div><button className="text-button" onClick={() => { setObserving(true); setMobilePanel('garden'); setFloats([]); setToast('') }}>观赏模式</button><button className="text-button" onClick={() => setPanel('help')}>玩法指南 <span>?</span></button></div>
        <nav className="garden-pages" aria-label="花园分页">{GARDENS.map((g,i)=><button key={g.name} disabled={i>=gardenCount(s)} aria-pressed={i===s.activeGarden} onClick={()=>{dispatch({type:"garden",index:i});setMoveFrom(null);setInspectedPot(null)}}><b>{i+1} · {g.name}</b><small>产值 ×{g.reward}{i>=gardenCount(s)?" · 待开辟":""}</small></button>)}</nav>
        <div className="garden-tools" role="group" aria-label="园艺工具">
          <div className="watering-tool"><button className={`tool-button ${tool === 'water' ? 'selected' : ''}`} aria-label="水壶工具" aria-pressed={tool === 'water'} onClick={() => { setTool(tool === 'water' ? null : 'water'); setMoveFrom(null) }}><Icon name="water" /><span>水壶</span></button><div className="tool-charge" aria-live="polite"><span data-testid="water-stock">{s.player.phase === 'service' ? '装填中' : '充能'} {s.player.stock}/{capacity(s, 'player')}</span><Progress value={s.player.phase === 'service' ? s.player.clock / 1.2 : s.player.stock / capacity(s, 'player')} /></div><button className="refill-button" aria-label="装满水壶充能" disabled={s.player.phase !== 'idle' || s.player.stock === capacity(s, 'player')} onClick={() => dispatch({ type: 'refill' })}>装满</button></div>
          <button className={`tool-button ${tool === "fertilizer" ? "selected" : ""}`} aria-label="肥料工具" aria-pressed={tool === "fertilizer"} onClick={()=>{setTool(tool === "fertilizer" ? null : "fertilizer");setMoveFrom(null)}}><Icon name="leaf" /><span>肥料<small>{s.fertilizer} 份</small></span></button>
          <button className={`tool-button ${tool === 'cart' ? 'selected' : ''}`} aria-label="小推车工具" aria-pressed={tool === 'cart'} onClick={() => { setTool(tool === 'cart' ? null : 'cart'); setMoveFrom(null) }}><Icon name="cart" /><span>小推车</span></button>
          <button className={`tool-button ${tool === 'shovel' ? 'selected' : ''}`} aria-label="铲子工具" aria-pressed={tool === 'shovel'} onClick={() => { setTool(tool === 'shovel' ? null : 'shovel'); setMoveFrom(null) }}><Icon name="shovel" /><span>铲子</span></button>
          <p className="tool-hint">{tool === 'fertilizer' ? '消耗 1 份，让未成熟的非终极植物立即成熟 · 收获时 1% 掉落' : tool === 'shovel' ? '点击挖除任何植物 · 无收益 · 再点铲子取消' : tool === 'cart' ? moveFrom === null ? '选择植物 → 点击空盆移动，或点击另一株交换位置' : `已选择花盆 ${moveFrom + 1} · 点击目的地，再点原盆取消` : tool === 'water' ? '每株冷却 1.2 秒 · 可连续浇不同植物 · 每次 1 格充能' : '点击空盆播种，点击成熟植物收获'}</p>
        </div>
        <div className={`garden-board garden-${garden.theme} wood zen-garden`}>
          {s.elapsed-s.weather.started>=0 && s.elapsed-s.weather.started<15 && <div className={`weather-overlay weather-${s.weather.kind}`} aria-label={`${WEATHER[s.weather.kind]}天气，持续15秒，无数值效果`}><span>{WEATHER[s.weather.kind]} · 15 秒</span>{Array.from({length:28},(_,i)=><i key={i} style={{left:`${i*37%100}%`,top:`${i*23%100}%`,animationDelay:`-${i*.19}s`}}/>)}</div>}
          <div className="garden-decorations" aria-hidden="true">{s.decorations.filter(id=>!s.hiddenDecorations.includes(id)).map(id=><div key={id} className={`decoration decoration-${id}`}><DecorationArt id={id}/></div>)}</div>
          ...
          <div className="garden-floor" inert={observing}><svg className="garden-landmark" viewBox="0 0 120 40" preserveAspectRatio={s.activeGarden === 1 ? "none" : "xMidYMid meet"} aria-hidden="true" shapeRendering="crispEdges">{s.activeGarden===0?<path fill="#476848" d="M12 36V8h4v28h-4M4 12h8v4H4m12 4h10v4H16m-12 2h8v4H4"/>:s.activeGarden===1?<><path fill="#428eaa" d="M0 22h26v-8h30v8h30v-9h34v17H86v7H56v-9H26v8H0Z"/><path fill="#a0dfd0" d="M0 23h26v-6h28v4H28v6H0m64 0h22v-6h24v4H90v6H64Z"/></>:s.activeGarden===2?<><path fill="#80552e" d="M41 29h38v8H41zM48 23h24v6H48z"/><path fill="#ead18b" d="M47 18h27v7H47z"/><path fill="#5e4530" d="M59 1h4v21h-4z"/></>:s.activeGarden===3?<><path fill="#58a4b1" d="M30 35V16l8-12 8 12v19zm24 0V12L65 0l11 12v23zm30 0V21l8-10 8 10v14z"/><path fill="#d2fcf0" d="M38 4v26h-4V16zm27-4v30h-5V12zm27 11v21h-4V21z"/></>:<><path fill="none" stroke="#c7a559" strokeWidth="3" d="M38 5h45v23H38zM50 0h20v36H50z"/><path fill="#f5db78" d="M56 12h10v10H56z"/><path fill="#66517c" d="M45 34h34v5H45z"/></>}</svg>
            ...
            if (!pot) return <button key={i} className="locked-pot ground-marker" style={placement} aria-label={`扩建第${i + 1}个花盆`} onClick={() => { setCategory(0); setMobilePanel('upgrades'); setToast('完成前三组改造后，在第四组购买「蜜糖苔庭扩建」。') }}><span>＋</span><small>扩建</small></button>
            const p = pot.plant === null ? null : PLANTS[pot.plant]
            const ready = p !== null && pot.growth >= p.seconds
            const hidden = isGerminating(pot)
            const seed = hidden || (p !== null && pot.growth < p.seconds * .15)
            const young = hidden || (p !== null && pot.growth < p.seconds * .5)
            const sprite = p === null || young ? null : p.id
            return <button key={i} data-testid={`pot-${i}`} aria-label={`花盆${i + 1} ${hidden ? '神秘种子' : p?.name ?? '空闲'} ${tool === 'fertilizer' ? '施肥' : tool === 'shovel' ? '挖除' : tool === 'cart' ? moveFrom === null ? '选择移动' : '移动到此处' : !p ? '播种' : ready ? '收获' : tool === 'water' ? '使用水壶' : '生长中'}`} className={`pot ${moveFrom === i ? 'move-source' : ''} ${tool === 'cart' && moveFrom !== null ? 'move-destination' : ''} ${(pot.watering ?? 0) > 0 ? 'watering-cooldown' : ''} ${ready ? 'ready' : ''} ${p === null ? 'empty-pot' : ''} ${p?.tier === ULTIMATE_TIER ? 'ultimate' : ''} ${p && !seed ? 'alive' : ''} plant-${hidden ? 'unknown' : p?.id ?? 'empty'}`} style={placement} onClick={() => potClick(i)}>

              {p && <span className="plant-name">{hidden ? "神秘种子" : `${pot.variant ? VARIANTS[pot.variant].name + ' · ' : ''}${p.name}`}</span>}
              {p && !young && p.tier > 0 && <span className={`plant-aura aura-${p.tier}`} aria-hidden="true"><i /><i /><i /></span>}
              <span key={`${sprite}-${ready}`} className="plant-art-slot">{sprite === null ? <Sprite id={10} className="pot-art" /> : <PlantSprite id={sprite} className={`pot-art ${VARIANTS[pot.variant ?? 0].className}`} />}</span>
              {hidden && <svg className="mystery-seed" viewBox="0 0 24 24" aria-hidden="true" shapeRendering="crispEdges"><path fill="#402719" d="M9 4h8v3h3v9h-3v4H7v-3H4V9h3V6h2Z"/><path fill="#d69b4a" d="M10 7h6v3h2v5h-3v3H8v-3H7v-5h3Z"/><path fill="#ffe6a0" d="M11 8h3v3h-3v4H8v-3h3Z"/></svg>}
              ...
              {p && young && !hidden && <Sprout id={p.id} tiny={seed} />}
              {(pot.watering ?? 0) > 0 && <span className="tool-pour" style={{ animationDuration: `${WATER_DURATION}s` }} aria-hidden="true"><Sprite id={14} /><span className="tool-water-stream">▪<i>▪</i><b>▪</b></span></span>}
              ...
              {floats.some(f => f.pot === i && f.kind === 'move') && <span className="tool-cart-animation" aria-hidden="true"><Icon name="cart" /></span>}
              ...
              <span className="pot-sign"><strong>{p ? ready ? '可收获' : hidden ? '萌发中' : seed ? '萌芽中' : young ? '生长中' : p.tier === ULTIMATE_TIER ? '凝聚星光' : '成株生长' : '空 闲'}</strong><Progress value={hidden ? (pot.germination ?? pot.growth) / GERMINATION_SECONDS : p ? pot.growth / p.seconds : 0} gold={p?.tier === ULTIMATE_TIER} /><small>{tool === 'shovel' ? p ? '挖除 · 无收益' : '空花盆' : p ? hidden ? '品种待揭晓' : ready ? `+${number(reward(s, p))} 金币` : `${formatTime((p.seconds - pot.growth) / growthRate(s))}` : '点击播种'}</small></span>
            ...
            <div className="supply-station water-station" style={{ left: `${STATIONS.water.x}%` }} aria-label="蜗牛补水池"><span className="pixel-pool" /><b>蜗牛水池</b></div>
            ...
            {([
              ...team.snails.slice(0, s.upgrades.snail ? 3 : 0).map((w, i) => ({ kind: 'water' as const, w, on: true, name: `浇水蜗牛${i + 1}`, level: s.upgrades.water })),
              ...(['harvest', 'sow'] as const).filter(kind => s.upgrades[kind] > 0).map(kind => ({ kind, w: team.workers[kind], on: kind === 'harvest' ? s.autoHarvest : s.autoSow && s.selected !== ULTIMATE_ID, name: kind === 'harvest' ? '收获甲虫' : '播种松鼠', level: s.upgrades[kind] - 1 }))
              ...
              <span className="cargo-pips" aria-hidden="true">{Array.from({ length: Math.min(8,capacity(s, kind)) }, (_, j) => <i key={j} className={j < (kind === 'harvest' ? w.count : w.stock) ? 'filled' : ''} />)}</span>
              {kind === 'water' && w.phase === 'act' && <span className="pour-stream">▪<i>▪</i><b>▪</b></span>}
              {on && (w.phase === 'act' || w.phase === 'service') && <span className="worker-action">{w.phase === 'service' ? kind === 'harvest' ? `交付 ${number(w.cargo)}` : '装填' : kind === 'harvest' ? '采摘' : kind === 'sow' ? '播种' : '浇水'}<Progress value={w.clock / workerDuration(s,w.phase === 'service')} gold={kind === 'harvest'} /></span>}
            </div>)}
          </div>
          <div className="garden-scene-caption"><small>{garden.subtitle} · 产值 ×{garden.reward} · 环境生长 ×{garden.growth}</small><span>{inspectedPot !== null && s.pots[inspectedPot] ? (() => { const pot = s.pots[inspectedPot]; const p = pot.plant === null ? null : PLANTS[pot.plant]; return isGerminating(pot) ? '神秘种子 · 经过 5 秒有效成长揭晓，浇水可加速' : p ? `${p.name} · ${pot.growth >= p.seconds ? '已成熟，点击收获' : `成长 ${Math.floor(pot.growth / p.seconds * 100)}%`} · 收获 ${reward(s, p)} 金币` : '空花盆 · 点击种下当前选择的种子' })() : '选择顶部工具，再点击盆栽使用'}</span><small>{s.upgrades.snail ? `3 只蜗牛在园中漫游` : '雇用蜗牛后，它会往返水池与花盆'}</small></div>
        </div>
      ...
      <aside className={`upgrade-panel ${mobilePanel === 'upgrades' ? 'mobile-active' : ''}`} aria-label="升级商店">
        ...
        <div className="upgrade-tabs" role="tablist" aria-label="升级分类">{['待购买', '下一组', '已获得', '装饰'].map((name, i) => <button role="tab" aria-selected={category === i} key={name} onClick={() => { setCategory(i); sound('tap') }} className={category === i ? 'selected' : ''}><Icon name={['hand', 'pot', 'snail', 'star'][i]} /><span>{name}</span></button>)}</div>
        <div className="upgrade-list">{category===3 && <><p className="decoration-note">纯外观 · 不增加通关进度<br/>购买后可随时隐藏，装饰所有花园</p>{DECORATIONS.map(d=><button key={d.id} className="upgrade-card" disabled={!s.decorations.includes(d.id)&&s.coins<d.cost} onClick={()=>{dispatch({type:s.decorations.includes(d.id)?'decoration-toggle':'decorate',id:d.id});sound('buy')}}><span className="upgrade-art"><Icon name="star"/></span><span className="upgrade-copy"><strong>{d.name}</strong><b>{s.decorations.includes(d.id)?s.hiddenDecorations.includes(d.id)?'已拥有 · 点击显示':'已展示 · 点击隐藏':`◈ ${number(d.cost)}`}</b><span className="upgrade-description">{d.detail}</span></span></button>)}</>}{UPGRADES.filter(u => category === 3 ? false : category === 2 ? s.purchases.includes(u.id) : u.chapter === chapter + category).map(u => {
          const level = s.purchases.includes(u.id) ? 1 : 0, max = level === 1, cost = upgradePrice(s, u), lock = max ? null : upgradeLock(s, u.id)
          return <button key={u.id} data-testid={`upgrade-${u.id}`} className={`upgrade-card ${lock ? 'upgrade-locked' : ''}`} disabled={max || !!lock || s.coins < cost} onClick={() => { dispatch({ type: 'buy', id: u.id }); setMoveFrom(null); setInspectedPot(null); sound('buy'); setToast(`${u.name} · ${u.detail}`) }}>
            <span className="upgrade-art"><Icon name={u.icon} /></span><span className="upgrade-copy"><strong>{u.name}</strong><span className="upgrade-price"><small>第 {u.chapter + 1} 组</small><b>{max ? '已获得' : `◈ ${number(cost)}`}</b></span><Progress value={level / u.max} /><span className="upgrade-description">{u.detail}</span>{lock && <span className="upgrade-requirement">解锁：{lock}</span>}</span>
          </button>
        })}</div>
        <div className="garden-bonuses"><p>独立改造 <b>{s.purchases.length} / 60</b></p><div className="eyebrow">花园加成</div><p><span>手动浇水</span><b>+{clickPower(s).toFixed(0)} 秒</b></p><p><span>自然生长</span><b>×{growthRate(s).toFixed(2)}</b></p><p><span>收获收益</span><b>×{2 ** s.upgrades.profit * garden.reward}</b></p><p><span>蜗牛助手</span><b>{s.upgrades.snail ? 3 * gardenCount(s) : 0} 只</b></p></div>
    ...
    {screen === 'menu' && !panel && <Modal title="月光奇植园开始菜单" className="start-menu">
      ...
      <small className="menu-footnote">21 种奇植 · 60 项改造 · 五座花园 · 慢慢种下自己的星空</small>
    </Modal>}
    ...
    {panel === 'help' && <Modal title="玩法指南" close={() => setPanel(null)}><h2>园丁的小手册</h2><ol className="help-list"><li><b>选种、播种</b><p>六档种子依次为普通、稀有、珍贵、超凡、神话、终极。前五档各有四种植物，普通种子免费；各档均能抽到全部二十种非终极植物，具体概率见商店。各品阶内四种植物概率依次为 20%、50%、25%、5%。终极种子固定种出星之花。前 5 秒有效成长只显示神秘种子，浇水与土壤可加速揭晓；灯笼初始成长不会跳过萌发。挖除不退种子费用。</p></li><li><b>浇水、收获</b><p>植物会自然生长。不同品种会长出不同幼芽。顶部选择水壶，再点击单株植物播放 1.2 秒浇水动画，期间该株无法重复浇水，但可以同时浇其他植物，每次立即消耗一格充能；点击水壶右侧「装满」补水。默认点击空盆播种、点击成熟植物收获，无需选择专门工具；水壶选中时也可直接播种和收获。铲子选中后可挖除任何阶段的植物，且不获得金币或收获次数。再次点击已选工具可取消选择。选择小推车后先点植物，再点空盆移动或另一株植物交换位置，成长进度会保留。手动或甲虫收获时均有 1% 概率获得一份肥料。选择肥料工具后点击未成熟的非终极植物，消耗一份立即养成；终极、空盆或已成熟植物不会消耗肥料。植物没有额外特殊效果；进度满后再次点击收获金币。</p></li><li><b>扩建、雇用助手</b><p>右侧有 60 项一次性改造，每完成三项开放下一组。浇水、产值、生长、容量有翻倍提升。最多开辟五座各有 15 盆的花园，后面的花园有更高产值。每页都有独立助手，切换页面后仍继续照料。松鼠自动选择已解锁的最高非终极种子，钱不足时播种免费种子。甲虫采摘后回站交付金币，蜗牛用完水后回池补水。</p></li><li><b>种出第一颗星星</b><p>累计收益解锁新种子。完成至少 57 项改造并开辟五座花园后，可解锁终极种子。星之花基础成长需 8 分钟，与其他植物一样接受手动浇水、蜗牛浇水、土壤和萤火灯笼加成。成熟即通关，之后可以继续种植。</p></li></ol><p className="settings-note">装饰仅改变外观，花钱购买后可随时隐藏。每隔 5–10 分钟出现一次细雨、萤火虫之夜或彩虹，持续 15 秒，不改变收益和成长。星之花不会被自动收获或自动播种。可关闭自动收获，保留喜欢的植物观赏。所有浇水只作用于单株植物。水壶空了请在顶部工具栏装填。观赏模式保留音乐和自动照料，隐藏数值提示与通关弹窗。</p><button className="primary-button" onClick={() => setPanel(null)}>知道了</button></Modal>}
    {panel === 'book' && <Modal title="植物图鉴" className="book-modal" close={() => setPanel(null)}><h2>奇植图鉴 <small>{harvestedKinds} / {PLANTS.length}</small></h2><p className="modal-intro">实际收获点亮品种；外观变体只要培育成熟即永久记录。薄荷 3%、霞光 2%、星尘 1%，独立于品种抽取，不改变收益。</p>{s.untrackedHarvests > 0 && <p className="settings-note">旧存档的 {number(s.untrackedHarvests)} 次收获未记录品种；各品种次数从本次更新后开始累计。</p>}<div className="book-grid">{TIER_PLANTS.flat().map(id => PLANTS[id]).map(p => {
      const count = s.harvestCounts[p.id], known = count > 0
      return <article key={p.id} data-testid={`book-plant-${p.id}`} className={known ? 'discovered' : 'undiscovered'}><PlantSprite id={p.id} /><div><h3>{known ? p.name : '未知植物'}<small>{TIERS[p.tier].replace('种子', '')} · {known ? '已收获' : '未收获'}</small></h3><div className="variant-collection">{VARIANTS.slice(1).map((v,i)=>{const collected=s.variants.includes(`${p.id}:${i+1}`);return <span key={v.name} className={collected?'variant-known':'variant-unknown'} title={collected?`${v.name} · 已培育成熟`:`${v.name} · 尚未培育`}><span className={collected?v.className:'variant-silhouette'}><PlantSprite id={p.id}/></span><small>{v.name} {collected?'✓':'?'}</small></span>})}</div><p className="harvest-count">累计收获 <b>{number(count)}</b> 次</p>{known ? <><p>基础成长 {formatTime(p.seconds)} · 收获 {number(p.reward)} 金币{p.id === ULTIMATE_ID ? ' · 成熟即通关' : ''}</p><span>{p.lore}</span></> : <span>收获后解锁名字与详细资料</span>}</div></article>
    })}</div></Modal>}
