    ...
    <main className="game-layout">
      ...
      <section className={`garden-section ${mobilePanel === 'garden' ? 'mobile-active' : ''}`} aria-label={text('花园','Garden')}>
        ...
        <div className="garden-tools" role="group" aria-label={text('园艺工具','Garden tools')}>
          ...
          <h2 className="scene-garden-name" aria-live="polite">
            ...
            <span className="scene-garden-effects" aria-label={text(`当前收获加成 ${multiplier(currentHarvestMultiplier)} 倍，生长速度 ${multiplier(currentGrowthMultiplier)} 倍`, `Current harvest multiplier ${multiplier(currentHarvestMultiplier)}, growth speed ${multiplier(currentGrowthMultiplier)}`)}>
              <span className="garden-effect harvest-effect">{text('收获','Harvest')} <strong>×{multiplier(currentHarvestMultiplier)}</strong></span>
              <span className="garden-effect growth-effect">{text('生长','Growth')} <strong>×{multiplier(currentGrowthMultiplier)}</strong></span>
            </span>
          </h2>
      ...
      <aside id="garden-shop" ref={shopRef} className={`upgrade-panel ${mobilePanel === 'upgrades' ? 'mobile-active' : ''}`} aria-label={text('花园商店','Garden shop')}>
        ...
        <div className={`upgrade-list ${category===1?'hire-list':''}`}>
          {category===0 && <>{research.map(u=>{const upgradeName=english?(u.icon==='coin'?`Harvest Study Lv. ${u.name.match(/\d+/)?.[0]}`:u.icon==='leaf'?`Soil Study Lv. ${u.page+1}`:`Infusion Study Lv. ${u.page+1}`):u.name;const profitIncrease=Math.round((2**(u.effects.profit??0)-1)*100);const upgradeDetail=u.icon==='coin'?text(`本次升级：收获 ×${multiplier(2**(u.effects.profit??0))}（+${profitIncrease}%）`,`This upgrade: harvest ×${multiplier(2**(u.effects.profit??0))} (+${profitIncrease}%)`):english?(u.icon==='leaf'?'This upgrade: growth speed ×2':'This upgrade: watering growth ×2 · capacity ×2'):u.detail;return <button key={u.id} data-testid={`upgrade-${u.id}`} className="upgrade-card" disabled={s.coins<upgradePrice(s,u)||!!upgradeLock(s,u.id)} onClick={()=>{dispatch({type:'buy',id:u.id});sound('buy');setToast(`${upgradeName} · ${upgradeDetail}`)}}><span className="upgrade-art"><Icon name={u.icon}/></span><span className="upgrade-copy"><strong>{upgradeName}</strong><b>◈ {number(u.cost)}</b><span className="upgrade-description">{lockText(upgradeLock(s,u.id))??upgradeDetail}</span></span></button>})}{UPGRADES.every(u=>s.purchases.includes(u.id))?<p className="shop-empty">{text('全园研究已满级。','All studies are complete.')}<br/>{text('可从花园右侧箭头开辟下一园。','Use the right garden arrow to open the next garden.')}</p>:research.length===0&&<p className="shop-empty">{text('继续经营花园，会出现新的研究项目。','Keep tending the garden to reveal new studies.')}</p>}</>}
          {category===1 && <>{hireCatalog(s.activeGarden).filter(u=>hireAvailable(team,u)).map(u=>{const hireName=english?(u.type==='recruit'?`${EN_CREW[u.kind]} ${u.level}`:`${EN_MATERIALS[u.level]} ${u.kind==='water'?'Tank':u.kind==='harvest'?'Basket':'Seed Bag'}`):u.name;const hireDetail=english?(u.type==='recruit'?'Adds 1 helper here':u.kind==='water'?'Watering and capacity ×2; faster':'Capacity ×2; faster'):u.detail;return <button key={u.id} data-testid={`hire-${u.id}`} className="upgrade-card" disabled={s.coins<u.cost||!!hireLock(s,u)} onClick={()=>{dispatch({type:'hire',id:u.id});sound('buy');setToast(`${gardenName(s.activeGarden)} · ${hireName}`)}}><span className="upgrade-art"><Icon name={u.kind==='water'?'snail':u.kind==='harvest'?'beetle':'squirrel'}/></span><span className="upgrade-copy"><strong>{hireName}</strong><b>◈ {number(u.cost)}</b><span className="upgrade-description">{lockText(hireLock(s,u))??hireDetail}</span></span></button>})}{hireCatalog(s.activeGarden).every(u=>!hireAvailable(team,u))&&<p className="shop-empty">{text('本园团队已满编，装备全部达到钻石制。','This garden’s team is full and all equipment is Diamond.')}</p>}</>}
