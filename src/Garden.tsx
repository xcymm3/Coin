...
export default function Garden({ initialState, persist = true }: { initialState?: GameState; persist?: boolean } = {}) {
  ...
  const nextGardenBlock = canOpenNextGarden ? expansionLock(s) ?? (s.coins < nextGardenPrice ? `金币不足 · 需要 ${number(nextGardenPrice)}` : null) : null
  const nextGardenLabel = canVisitNextGarden ? `下一座花园 · ${GARDENS[s.activeGarden+1].name}` : canOpenNextGarden ? `开辟${GARDENS[openGardenIndex].name} · ${nextGardenBlock ?? `花费 ${number(nextGardenPrice)} 金币`}` : '已是最后一座花园'
  ...
  function potClick(index: number) {
    ...
    if (p.plant === null && !(tool === 'cart' && moveFrom !== null)) {
      ...
      sound('plant')
      setFloats(f => [...f.slice(-9), { id: floatId.current++, pot: index, text: '种下了！', kind: 'plant' }])
    ...
    let text = ''
    if (p.plant !== null && p.growth >= PLANTS[p.plant].seconds) { text = `+${number(plantedReward(s,index))}`; sound('coin') }
    else { setToast('选择顶部水壶，再点击植物浇水。'); return }
    setFloats(f => [...f.slice(-9), { id: floatId.current++, pot: index, text, kind: 'coin' }])
  ...
  function advanceGarden() {
    ...
    if (nextGardenBlock) {
      setToast(`暂不能开辟${GARDENS[openGardenIndex].name} · ${nextGardenBlock}`)
      sound('tap')
      return
    }
    ...
    <main className="game-layout">
      ...
      <section className={`garden-section ${mobilePanel === 'garden' ? 'mobile-active' : ''}`} aria-label="花园">
        ...
        <div className="garden-tools" role="group" aria-label="园艺工具">
          ...
          <p className="tool-hint">{tool === 'fertilizer' ? '点击植物施肥 · 点击空盆仍播种当前种子' : tool === 'shovel' ? '点击植物挖除 · 点击空盆仍播种当前种子 · 再点铲子取消' : tool === 'cart' ? moveFrom === null ? '选择植物搬运 · 点击空盆仍播种当前种子' : `已选 ${GARDENS[Math.floor(moveFrom/15)].name} · ${moveFrom%15+1} 号盆，可跨园搬运；再点小推车取消` : tool === 'water' ? '点击植物浇水 · 点击空盆仍播种当前种子 · 点击左下角水池打水' : '点击空盆播种，点击成熟植物收获'}</p>
        </div>
